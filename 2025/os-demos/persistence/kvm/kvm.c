...
int main() {
    ...
    /* Set vCPU special registers (real mode) */
    ...
    /* Set vCPU general-purpose registers */
    memset(&regs, 0, sizeof(regs));
    regs.rip    = GUEST_ENTRY;   /* point RIP to where we loaded the code */
    regs.rax    = 2;
    regs.rbx    = 2;
    regs.rflags = 0x2;
    ...
    while (1) {
        ...
        case KVM_EXIT_IO:
            ...
            break;
        case KVM_EXIT_MMIO:
            printf("KVM_EXIT_MMIO: address=0x%llx, data=0x%llx\n",
                   (unsigned long long)run->mmio.phys_addr,
                   *(unsigned long long *)run->mmio.data);
            break;
