...
void dl_objdump(const char *path) {
    ...
    for (char *code = hc + h->hdr.code_off; code < hc + h->hdr.file_sz;
         code++) {
        for (struct symbol *sym = h->symtab; sym->type; sym++) {
...
int main(int argc, char *argv[]) {
    ...
    for (struct cmd *cmd = &commands[0]; cmd->handler; cmd++) {
        for (char **path = &argv[2]; *path && strcmp(argv[1], cmd->cmd) == 0;
             path++) {
...
static struct dlib *dlopen(const char *path) {
    ...
    h = mmap(NULL, hdr.file_sz, PROT_READ | PROT_WRITE | PROT_EXEC, MAP_PRIVATE,
             fd, 0);
...
static void *dlsym(const char *name) {
    for (int i = 0; i < LENGTH(syms); i++)
        if (strcmp(syms[i].name, name) == 0) return (void *)syms[i].offset;
    assert(0);
}
