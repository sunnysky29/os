...
def sys_fork(): return os.sys_fork()


sys_spawn = lambda fn, *args: os.sys_spawn(fn, *args)
def sys_sched(): return os.sys_sched()

# 1.2 Virtual character device


def sys_choose(choices): return os.sys_choose(choices)
...
def sys_bread(k): return os.sys_bread(k)
def sys_bwrite(k, v): return os.sys_bwrite(k, v)
def sys_sync(): return os.sys_sync()
def sys_crash(): return os.sys_crash()
    ...
    def sys_bread(self, key):
        ...
        return {'bread': (lambda:
                          storage.buf.get(key,  # always try to read from buffer first
                                          # and then persistent storage
                                          storage.persist.get(key, None)
                                          )
                          )}
